...
class OrderSystem:
    ...
    def process(self):
        ...
        for store in ['opticalh', 'gafascanarias']:
            # process_store es un generador, necesitamos consumirlo
            store_count = 0
            for msg in self.process_store(store):
                yield msg
                # El último yield de process_store no tiene count, lo manejamos diferente
            
            # Contar pedidos procesados basándose en los mensajes de éxito
            # (esto es una simplificación, el count real se maneja internamente)
